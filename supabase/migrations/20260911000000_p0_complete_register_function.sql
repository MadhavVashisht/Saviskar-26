-- ============================================================
-- SAVISKAR 2026 — P0 CORRECTIVE MIGRATION
-- ============================================================
-- Filename: 20260911000000_p0_complete_register_function.sql
--
-- Purpose:
--   Migration 20260831000000_p0_deadlock_fix.sql introduced a
--   correct deterministic event-lock ordering (ORDER BY event_id UUID)
--   but inadvertently used an older baseline of
--   register_participant_events that was missing:
--
--   - Structured application error codes (SVK01-SVK11)
--   - registration_limit enforcement
--   - Participant ID collision-retry loop
--   - Atomic payment_orders row creation
--   - Atomic payment_order_items row creation
--
--   This migration provides the definitive merged function that
--   preserves BOTH the deadlock fix and the complete payment/
--   integrity logic from 20260830180000.
--
-- Safety:
--   - Uses CREATE OR REPLACE -- idempotent and non-destructive.
--   - Does NOT modify any existing migration files.
--   - Does NOT delete any data.
--   - Preserves SECURITY DEFINER, search_path, and all grants.
-- ============================================================
