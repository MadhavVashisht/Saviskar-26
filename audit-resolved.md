...
- `npm run test`: **18 test files, 252 tests passing** (Exit code 0)
- `npx playwright test`: **15 E2E tests passing** (Exit code 0, 0 console errors)
- `npm run build`: **Compiled successfully in 1.7s, 33/33 static/dynamic routes generated** (Exit code 0)

---

## 1. Milestone Execution & Commit Mapping

The remediation was executed across 9 structured milestones, each mapped to a dedicated commit:

- **Milestone 1 (`66b3e49`) — Build Stability & Prerender Crash Elimination:**
  Resolved `/legacy` SSR crash, added amber palette tokens, escaped JSX apostrophes/comments, and eliminated lying type casts.
- **Milestone 2 (`4fc42f1`) — Payment Integrity, Idempotency & Gate Check-in:**
  Deferred QR pass issuance until payment verified, implemented `processed_payment_events` idempotency with Postgres 23505 handling, and added atomic single-use check-in.
- **Milestone 3 (`b97bf5b`) — Asset Compression & WebGL Performance:**
  Compressed 8K textures to responsive WebP, optimized 175 dome gallery tiles, enabled WebGL texture disposal, and added ISR (`revalidate = 300`).
- **Milestone 4 (`3ea5deb`) — Error Boundaries & Network Resilience:**
  Added root `error.tsx`, route-level `loading.tsx`, dark festival-themed 404 page, and offline retry mechanisms.
- **Milestone 5 (`03802f5`) — Responsive Layouts & Mobile Viewports:**
  Fixed horizontal overflow, adapted dynamic viewport heights (`dvh`), and disabled custom trailing cursor on touch devices.
- **Milestone 6 (`41af8e7`) — Accessibility (a11y) & Reduced Motion:**
  Eliminated keyboard traps in 3D dome gallery with Enter/Space keyboard navigation and implemented `prefers-reduced-motion` fallbacks.
- **Milestone 7 (`7fa8bf3`) — Automated Vitest Regression & CI Pipeline:**
  Created 18 Vitest test suites (252 tests passing) covering registration invariants, admin permissions, and concurrency, plus GitHub Actions CI workflow.
- **Milestone 8 (`88a4db2`) — Security Hardening, CSP & Zero-Warning Lint Cleanup:**
  Enforced Zod schemas on API route handlers, configured Content Security Policy headers, and cleared all 145 ESLint errors/warnings to 0 with 0 `eslint-disable`.
- **Milestone 9 (`dc19daf`) — Production Telemetry, Health Check & Runbooks:**
  Added Sentry error instrumentation, `/api/health` probe endpoint, and `DEPLOYMENT.md` operational runbook.
...
| **P0-03** | **Critical** | Performance & Bandwidth | **RESOLVED** | `b97bf5b` | `components/ui/ScrollEngine3D.tsx`, `components/ui/Preloader.tsx`, `components/ui/DomeGallery.tsx`, `public/gallery/tiles/` | Compressed 5 stadium textures and 175 dome tiles. Reduced landing page network transfer from **120+ MB** to **2.97 MB** (STRICTLY ≤ 3.0 MB budget) and GPU VRAM usage from **~870 MB** to **~96 MB**. |
| **P0-04** | **High** | Animation & Performance | **RESOLVED** | `b97bf5b` | `components/ui/ScrollEngine3D.tsx` | Added `try/catch` around `new THREE.WebGLRenderer`, WebGL support detection, `webglcontextlost` handling, and fallback CSS radial background for devices without hardware acceleration. |
| **P0-05** | **High** | Supabase & Data Layer | **RESOLVED** | `4fc42f1` | `supabase/schema.sql`, `supabase/migrations/20260919060000_create_registration_otps.sql` | Added `registration_otps` table definition, created migration, revoked `anon` execution on `register_participant_events`, and restricted table access to `service_role`. Verified by `__tests__/auth/auth-flow.test.ts`. |
| **P0-06** | **High** | App Router Usage | **RESOLVED** | `b97bf5b` | `app/events/page.tsx`, `app/events/[category]/page.tsx`, `app/events/[category]/[event]/page.tsx`, `app/schedule/page.tsx` | Converted client fetch waterfalls to Server Components with ISR (`revalidate = 300`), added dynamic `generateMetadata()` with OpenGraph and Twitter cards, and eliminated client Supabase imports in server files. |
| **P0-07** | **High** | Resilience & Rate Limiting | **RESOLVED** | `4fc42f1` | `lib/send-registration-email.ts`, `lib/payments/post-payment.ts` | Replaced unbounded sequential `resend.emails.send` with throttled batch concurrency (`p-limit` 2 req/s) with exponential backoff on HTTP 429 and fail-safe error isolation so one member failure does not fail the transaction. |
| **P0-08** | **High** | Accessibility | **RESOLVED** | `41af8e7` | `components/ui/DomeGallery.tsx` | Implemented `onKeyDown` handlers for `Enter` and `Space` on 3D tiles, added `aria-label`, and skipped off-screen / non-visible tiles from keyboard tab stops to prevent keyboard traps (WCAG 2.1.1 & 2.1.2 compliance). |
| **P0-09** | **High** | Architecture & Stability | **RESOLVED** | `b97bf5b` | `components/ui/ScrollEngine3D.tsx` | Completely removed global mutations on `WebGL2RenderingContext.prototype.texImage3D` and `texSubImage3D`. Renderer now isolates textures cleanly without polluting the global environment. |
| **P1-10** | **Medium** | Security & Scalability | **RESOLVED** | `4fc42f1` | `lib/rate-limit.ts` | Upgraded in-memory sliding window rate limiter to support shared distributed keys, fallback sliding window algorithm, and IP hash headers for multi-instance Vercel edge/serverless execution. |
...
| **P1-13** | **Medium** | Error Handling & Resilience | **RESOLVED** | `3ea5deb` | `app/error.tsx`, `app/events/loading.tsx`, `app/schedule/loading.tsx`, `app/global-error.tsx` | Added root-level `error.tsx` boundary with ambient festival recovery UI, route-level `loading.tsx` skeleton states, and integrated `captureException` telemetry in global error handlers. |
...
| **P2-15** | **Low** | Accessibility & A11y | **RESOLVED** | `41af8e7` | `components/ui/ScrollEngine3D.tsx`, `components/ui/DomeGallery.tsx` | Added listener for `window.matchMedia("(prefers-reduced-motion: reduce)")`. Disables 3D rotation acceleration, particle vortexes, and chromatic aberration when reduced motion is preferred. |
...
| **Automated Vitest Tests** | 0 test files / 0 tests | **18 test files / 252 tests passing** | Comprehensive regression coverage |
| **Playwright E2E Suite** | 0 test files / 0 tests | **15 tests passing** | Full-site smoke + registration + check-in |
| **Landing Route Payload Transfer** | 120+ MB | **2.97 MB** (STRICTLY ≤ 3.0 MB budget) | **97.5% network bandwidth reduction** |
